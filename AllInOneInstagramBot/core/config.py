...
class Config:
    def __init__(self, first_run=False, **kwargs):
        ...
        self.parser = configargparse.ArgumentParser(
            ...
            description="AllInOneInstagramBot",
        )
    ...
    def parse_args(self):
        def _is_legacy_arg(arg):
            if arg in ["interact", "hashtag-likers"]:
                if self.first_run:
                    logger.warning(
                        f"You are using a legacy argument {arg} that is no longer supported. It will not be used. Please refer to https://www.allinoneinstagrambot.com/configuration-file#h.790ybcba7foc."
                    )
                return True
            return False
